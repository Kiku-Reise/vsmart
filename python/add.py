...
startNumber = int(input(f'{INPUT}{cy} Account Start: {r}'))
...
numberUserStart = startNumber;
abbStart = 0
for acc in to_use:
    abbStart += 1
    if abbStart < startNumber:
        continue
    stop = index + 60
    c = TelegramClient(f'sessions/{acc[0]}', 3910389 , '86f861352f0ab76a251866059a6adbd6')
    #c = TelegramClient(f'sessions/+84907499128', 3910389 , '86f861352f0ab76a251866059a6adbd6')
    ...
    try: 
        if '/joinchat/' in scraped_grp:
            ...
            try:
                ...
                print(f'Da vao group lay member') 
        ...
        if choice == 0:
            ...
            print(f'Da vao group add member') 
        ...
        print(f'Khong the join vao group -- Failed to join group')
        print(f'{error} {r}{e}')
        continue
    ...
    try:
        ...
        print(f'Khong the tai danh sach member')
        print(f'{error}{r} {e}')
        continue
    ...
    for user in members[index:stop]:
        ...
        try:
            if status == "online":
                ...
                #c.send_message(entity=target_entity.id, message='Welcome @'+user_id+' join crypto network. Now you have access airdrop get free coin')
                print(f'{plus}{grey} User: {numberUserStart} : {cy}{acc_name}{lg} add {cy}{user_id} {lg} to {cy}{target_title}')
                #print(f'{info}{grey} User: {cy}{acc_name}{lg} -- Sleep 1 second')
                adding_status += 1
                print(f'{info}{grey} User: {numberUserStart} : {cy}{acc_name}{lg} -- Sleep {w}{sleep_time} {lg}second(s)')
